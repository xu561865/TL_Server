...
--任务是否已经完成
x211002_g_IsMissionOkFail = 0		--变量的第0位

--以上是动态**************************************************************

--任务需要得到的物品
x211002_g_DemandItem={{id=40002059,num=1}}		--变量第1位

--任务文本描述
x211002_g_MissionName="灰熊王"
x211002_g_MissionInfo="鲁平只佩服比他更强大的人，抢先杀死灰熊王得到熊王的皮，就能得到鲁平的关注！"
x211002_g_MissionTarget="得到灰熊王的皮"
x211002_g_ContinueInfo="你已经得到灰熊王的皮了吗？"
x211002_g_MissionComplete="恭喜你得到了灰熊王的皮"
...
function x211002_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x211002_g_MissionId)			--得到任务的序列号
...
function x211002_OnKillObject( sceneId, selfId, objdataId, objId)
	if objdataId == 514 then
		...
		for i=0,num-1 do
			...
			if IsHaveMission(sceneId,humanObjId,x211002_g_MissionId) > 0 then	--如果这个人拥有任务	
				if (GetItemCount(sceneId,humanObjId,x211002_g_ItemId) < x211002_g_ItemNeedNum) then
					AddMonsterDropItem(sceneId,objId,humanObjId,x211002_g_ItemId)    --给这个人任务道具(道具会出现在尸体包里)
				end
			end
		end
	end
...
function x211002_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x211002_g_ItemId then
		...
		if x211002_g_ItemNum < x211002_g_ItemNeedNum then
			BeginEvent(sceneId)
				strText = format("已得到熊王的皮%d/1", x211002_g_ItemNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			--取得任务变量的值
		...
		elseif x211002_g_ItemNum == x211002_g_ItemNeedNum then
			...
			--显示得到物品数量
			BeginEvent(sceneId)
				strText = format("已得到熊王的皮%d/1", x211002_g_ItemNeedNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
