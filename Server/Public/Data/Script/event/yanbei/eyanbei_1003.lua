...
--任务是否已经完成
x211003_g_IsMissionOkFail = 0		--变量的第0位

--以上是动态**************************************************************

--任务需要得到的物品
x211003_g_DemandItem={{id=40002060,num=1}}		--变量第1位

--任务文本描述
x211003_g_MissionName="红枫蜘蛛王"
x211003_g_MissionInfo="鲁平只佩服比他更强大的人，抢先杀死红枫蜘蛛王得到蜘蛛王的丝，就能得到鲁平的关注！"
x211003_g_MissionTarget="得到红枫蜘蛛王的丝"
x211003_g_ContinueInfo="你已经得到红枫蜘蛛王的丝了吗？"
x211003_g_MissionComplete="恭喜你得到了红枫蜘蛛王的丝"
...
function x211003_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x211003_g_MissionId)			--得到任务的序列号
...
function x211003_OnKillObject( sceneId, selfId, objdataId, objId )
	if objdataId == 515 then
		...
		for i=0,num-1 do
			...
			if IsHaveMission(sceneId,humanObjId,x211003_g_MissionId) > 0 then	--如果这个人拥有任务	
				if (GetItemCount(sceneId,humanObjId,x211003_g_ItemId) < x211003_g_ItemNeedNum) then
					AddMonsterDropItem(sceneId,objId,humanObjId,x211003_g_ItemId)    --给这个人任务道具(道具会出现在尸体包里)
				end
			end
		end
	end
...
function x211003_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x211003_g_ItemId then
		...
		if x211003_g_ItemNum < x211003_g_ItemNeedNum then
			BeginEvent(sceneId)
				strText = format("已得到红枫蜘蛛王的丝%d/1", x211003_g_ItemNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			--取得任务变量的值
		...
		elseif x211003_g_ItemNum == x211003_g_ItemNeedNum then
			...
			--显示得到物品数量
			BeginEvent(sceneId)
				strText = format("已得到红枫蜘蛛王的丝%d/1", x211003_g_ItemNeedNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
