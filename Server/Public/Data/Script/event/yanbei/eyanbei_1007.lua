...
--任务是否已经完成
x211007_g_IsMissionOkFail = 0		--变量的第0位

--任务需要杀死的怪
x211007_g_DemandKill ={{id=1705,num=10}}		--变量第1位

--以上是动态**************************************************************

--任务文本描述
x211007_g_MissionName="混乱的教头"
x211007_g_MissionInfo="干掉10个秦家寨教头"
x211007_g_MissionTarget="干掉10个秦家寨教头"
x211007_g_ContinueInfo="你已经杀了10个秦家寨教头了吗？"
x211007_g_MissionComplete="恭喜你杀完了"
...
function x211007_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	AddMission( sceneId,selfId, x211007_g_MissionId, x211007_g_ScriptId, 1, 0, 0 )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x211007_g_MissionId)			--得到任务的序列号
...
function x211007_OnKillObject( sceneId, selfId, objdataId )
	...
	if objdataId == x211007_g_DemandKill[1].id then
		...
		--设置打怪数量+1
			...
			BeginEvent(sceneId)
				strText = format("已杀死秦家寨教头%d/10", GetMissionParam(sceneId,selfId,misIndex,1) )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
