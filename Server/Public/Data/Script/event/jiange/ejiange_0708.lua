...
--任务是否已经完成
x210708_g_IsMissionOkFail = 0		--变量的第0位

--任务需要杀死的怪
x210708_g_DemandKill ={{id=550,num=1}}		--所需杀死怪物id
x210708_g_DemandTrueKill ={{name="修罗小鬼",num=1}}		--所需杀死怪物名称

--******以上是动态******

--任务文本描述
x210708_g_MissionName="修罗小鬼"
x210708_g_MissionInfo="杀死修罗小鬼"
x210708_g_MissionTarget="杀死修罗小鬼"
x210708_g_ContinueInfo="你已经杀死修罗小鬼？没有就继续吧"
x210708_g_MissionComplete="恭喜你杀完了"
...
--任务入口函数
--**********************************
function x210708_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x210708_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	AddMission( sceneId,selfId, x210708_g_MissionId, x210708_g_ScriptId, 1, 0, 0 )	--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x210708_g_MissionId)	--得到任务的序列号
...
function x210708_OnKillObject( sceneId, selfId, objdataId ,objId)
	if GetName(sceneId,objId) == x210708_g_DemandTrueKill[1].name	  then
		...
		if num < x210708_g_DemandTrueKill[1].num then
			...
			BeginEvent(sceneId)
				strText = format("已杀死修罗小鬼%d/1", GetMissionParam(sceneId,selfId,misIndex,1) )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
