...
--任务是否已经完成
x210701_g_IsMissionOkFail = 0		--变量的第0位

--任务需要杀死的怪
x210701_g_DemandKill ={{id=1510,num=10}}		--变量第1位


--任务文本描述
x210701_g_MissionName="蜀道黑猿"
x210701_g_MissionInfo="杀死10个蜀道黑猿"
x210701_g_MissionTarget="杀死10个蜀道黑猿"
x210701_g_ContinueInfo="你已经杀了10只蜀道黑猿？没有就继续吧"
x210701_g_MissionComplete="恭喜你杀完了"
...
--任务入口函数
--**********************************
function x210701_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x210701_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	AddMission( sceneId,selfId, x210701_g_MissionId, x210701_g_ScriptId, 1, 0, 0 )	--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x210701_g_MissionId)	--得到任务的序列号
...
function x210701_OnKillObject( sceneId, selfId, objdataId )
	...
	if objdataId == x210701_g_DemandKill[1].id then
		...
		BeginEvent(sceneId)
		strText = format("已杀死蜀道黑猿%d/10", GetMissionParam(sceneId,selfId,misIndex,1) )
		--print(strText)
		AddText(sceneId,strText)
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
