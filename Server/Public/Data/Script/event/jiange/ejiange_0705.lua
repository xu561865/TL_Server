...
--任务是否已经完成
x210705_g_IsMissionOkFail = 0		--变量的第0位

--******以上是动态*****

--任务需要得到的物品
x210705_g_DemandItem={{id=40002106,num=1}}		--变量第1位

--任务文本描述
x210705_g_MissionName="蓝色宝藏钥匙"
x210705_g_MissionInfo="杀死草人首领，拿到蓝色宝藏钥匙"  --任务描述
x210705_g_MissionTarget="杀死草人首领，拿到蓝色宝藏钥匙"		--任务目标
x210705_g_ContinueInfo="还没有完成，需要继续努力哟"		--未完成任务的npc对话
x210705_g_MissionComplete="嗯？看来你有两下子嘛。"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x210705_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
	--上一个任务没有完成，return
...
function x210705_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x210705_g_MissionId)		--得到任务的序列号
...
function x210705_OnKillObject( sceneId, selfId, objdataId ,objId)
	...
	if IsHaveMission(sceneId,humanObjId,x210705_g_MissionId) > 0 then	--如果这个人拥有任务	
		if (GetItemCount(sceneId,humanObjId,x210705_g_ItemId) < x210705_g_ItemNeedNum) then
		AddMonsterDropItem(sceneId,objId,humanObjId,x210705_g_ItemId)    --给这个人任务道具(道具会出现在尸体包里)
				end
			end
		end
	end
...
function x210705_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x210705_g_ItemId then
		...
		if x210705_g_ItemNum < x210705_g_ItemNeedNum then
			BeginEvent(sceneId)
				strText = format("已得到蓝色宝藏钥匙%d/1", x210705_g_ItemNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			--取得任务变量的值
		...
		elseif x210705_g_ItemNum == x210705_g_ItemNeedNum then
			...
			--显示得到物品数量
			BeginEvent(sceneId)
				strText = format("已得到蓝色宝藏钥匙%d/1", x210705_g_ItemNeedNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
