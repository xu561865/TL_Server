...
--任务是否已经完成
x211307_g_IsMissionOkFail = 0		--变量的第0位

--以上是动态**************************************************************

--任务需要得到的物品
x211307_g_DemandItem={{id=40002098,num=1}}		--变量第1位

--任务文本描述
x211307_g_MissionName="神医的配方"
x211307_g_MissionInfo="我真的对不起叔叔，是我把神医的配方弄丢了，我记得是在雪人居住地的附近玩的时候弄丢了，如果让雪人得到，我怎么才能拿回来呢"  --任务描述
x211307_g_MissionTarget="帮完颜兀术把神医的配方从雪人手中夺回"		--任务目标
x211307_g_ContinueInfo="我怎么办，雪人太可怕了，我怎么弄回配方救我叔叔啊"		--未完成任务的npc对话
x211307_g_MissionComplete="什么，配方，你怎么得到的，你把雪人打败了？"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x211307_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x211307_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x211307_g_MissionId)			--得到任务的序列号
...
--杀死怪物或玩家
--**********************************
function x211307_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物objId
	if objdataId == 1940 then
		...
		for i=0,num-1 do
			...
			if IsHaveMission(sceneId,humanObjId,x211307_g_MissionId) > 0 then	--如果这个人拥有任务	
				if (GetItemCount(sceneId,humanObjId,x211307_g_ItemId) < x211307_g_ItemNeedNum) then
					AddMonsterDropItem(sceneId,objId,humanObjId,x211307_g_ItemId)    --给这个人任务道具(道具会出现在尸体包里)
				end
			end
		end
	end
...
function x211307_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x211307_g_ItemId then
		...
		if x211307_g_ItemNum < x211307_g_ItemNeedNum then
			BeginEvent(sceneId)
				strText = format("已得到神医的配方%d/1", x211307_g_ItemNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			--取得任务变量的值
		...
		elseif x211307_g_ItemNum == x211307_g_ItemNeedNum then
			...
			--显示得到物品数量
			BeginEvent(sceneId)
				strText = format("已得到神医的配方%d/1", x211307_g_ItemNeedNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
