...
--任务是否已经完成
x211303_g_IsMissionOkFail = 0		--变量的第0位

--以上是动态**************************************************************

--任务需要得到的物品
x211303_g_DemandItem={{id=40002096,num=5}}		--变量第1位

--任务文本描述
x211303_g_MissionName="熊胆"
x211303_g_MissionInfo="我听说熊胆能活血解毒，请帮我弄来5个熊胆，我要送给叔叔治病"  --任务描述
x211303_g_MissionTarget="给完颜斡离不带回5个熊胆"		--任务目标
x211303_g_ContinueInfo="嗨，你答应带给我的熊胆呢？你不是在撒谎吧？"		--未完成任务的npc对话
x211303_g_MissionComplete="哇，真棒，看来你真的没有撒谎"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x211303_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x211303_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x211303_g_MissionId)			--得到任务的序列号
...
--杀死怪物或玩家
--**********************************
function x211303_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物objId
	if objdataId == 1935 then
		...
		for i=0,num-1 do
			...
			if IsHaveMission(sceneId,humanObjId,x211303_g_MissionId) > 0 then	--如果这个人拥有任务	
				if (GetItemCount(sceneId,humanObjId,x211303_g_ItemId) < x211303_g_ItemNeedNum) then
					AddMonsterDropItem(sceneId,objId,humanObjId,x211303_g_ItemId)    --给这个人任务道具(道具会出现在尸体包里)
				end
			end
		end
	end
...
function x211303_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x211303_g_ItemId then
		...
		if x211303_g_ItemNum < x211303_g_ItemNeedNum then
			BeginEvent(sceneId)
				strText = format("已得到熊胆%d/5", x211303_g_ItemNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			--取得任务变量的值
		...
		elseif x211303_g_ItemNum == x211303_g_ItemNeedNum then
			...
			--显示得到物品数量
			BeginEvent(sceneId)
				strText = format("已得到熊胆%d/5", x211303_g_ItemNeedNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
