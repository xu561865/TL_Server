...
--任务需要得到的物品
x212107_g_DemandItem={{id=40002077,num=1}}		--变量第1位

--任务文本描述
x212107_g_MissionName="柳笛"
x212107_g_MissionInfo="杀死白堤柳鬼，得到柳笛"  --任务描述
x212107_g_MissionTarget="杀死白堤柳鬼，得到柳笛"		--任务目标
x212107_g_ContinueInfo="你已经得到柳笛了么？"		--未完成任务的npc对话
x212107_g_MissionComplete="干的不错"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x212107_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
--杀死怪物或玩家
--**********************************
function x212107_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物objId
	if objdataId == 1635 then
		...
		for i=0,num-1 do
			...
			if IsHaveMission(sceneId,humanObjId,x212107_g_MissionId) > 0 then	--如果这个人拥有任务	
				if (GetItemCount(sceneId,humanObjId,x212107_g_ItemId) < x212107_g_ItemNeedNum) then
					AddMonsterDropItem(sceneId,objId,humanObjId,x212107_g_ItemId)    --给这个人任务道具(道具会出现在尸体包里)
				end
			end
		end
	end
...
function x212107_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x212107_g_ItemId then
		...
		if x212107_g_ItemNum < x212107_g_ItemNeedNum then
			BeginEvent(sceneId)
				strText = format("已得到柳笛%d/1", x212107_g_ItemNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			--取得任务变量的值
		...
		elseif x212107_g_ItemNum == x212107_g_ItemNeedNum then
			...
			--显示得到物品数量
			BeginEvent(sceneId)
				strText = format("已得到柳笛%d/1", x212107_g_ItemNeedNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
