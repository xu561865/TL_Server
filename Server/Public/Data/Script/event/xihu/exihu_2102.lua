...
--打怪时需要带在身上的道具编号
x212102_g_ItemId1 = 40002083		--灵魂探测器

--打怪时需要带在身上的道具的数量
x212102_g_ItemNeedNum1 = 1

--任务道具编号
x212102_g_ItemId2 = 40002084		--旱船控制器
...
--任务是否已经完成
x212102_g_IsMissionOkFail = 0		--变量的第0位

--以上是动态**************************************************************

--任务需要得到的物品
x212102_g_DemandItem={{id=40002083,num=1},{id=40002084,num=1}}		--变量第1位

--任务文本描述
x212102_g_MissionName="旱船的秘密"
x212102_g_MissionInfo="带上这个灵魂探测器，去打败旱船，你就可以得到旱船的控制器"  --任务描述
x212102_g_MissionTarget="带上探测器去打败旱船"		--任务目标
x212102_g_ContinueInfo="你得到控制器了么"		--未完成任务的npc对话
x212102_g_MissionComplete="干得不错！"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x212102_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x212102_OnAccept( sceneId, selfId )
	...
	if ret > 0 then
		...
		BeginEvent(sceneId)
				strText = format("背包已满，无法接受任务", x212102_g_ItemNeedNum2 )
...
--杀死怪物或玩家
--**********************************
function x212102_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物objId
	if objdataId == 1625 then
		...
		for i=0,num-1 do
			...
			if IsHaveMission(sceneId,humanObjId,x212102_g_MissionId) > 0 then	--如果这个人拥有任务	
				if (GetItemCount(sceneId,humanObjId,x212102_g_ItemId2) < x212102_g_ItemNeedNum2) then	--检测是否已经拿到足够的任务物品
					if (HaveItem (sceneId,humanObjId,x212102_g_ItemId1) > 0) then	--检测身上是否带有需要带的任务道具
						AddMonsterDropItem(sceneId,objId,humanObjId,x212102_g_ItemId2)    --给这个人任务道具(道具会出现在尸体包里)
					end
				end
			end
		end
...
function x212102_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x212102_g_ItemId2 then
		...
		if x212102_g_ItemNum < x212102_g_ItemNeedNum2 then
			BeginEvent(sceneId)
				strText = format("已得到旱船的控制器%d/1", x212102_g_ItemNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			--取得任务变量的值
		...
		elseif x212102_g_ItemNum == x212102_g_ItemNeedNum2 then
			...
			--显示得到物品数量
			BeginEvent(sceneId)
				strText = format("已得到旱船的控制器%d/1", x212102_g_ItemNeedNum2 )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
