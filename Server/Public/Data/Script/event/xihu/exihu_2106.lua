...
--任务是否已经完成
x212106_g_IsMissionOkFail = 0		--变量的第0位

--任务需要杀死的怪
x212106_g_DemandKill ={{id=1640,num=10}}		--变量第1位

--以上是动态**************************************************************

--任务文本描述
x212106_g_MissionName="扬子鳄"
x212106_g_MissionInfo="杀死10只扬子鳄"  --任务描述
x212106_g_MissionTarget="杀死10只扬子鳄"		--任务目标
x212106_g_ContinueInfo="你已经杀死10只扬子鳄了么？"		--未完成任务的npc对话
x212106_g_MissionComplete="干的不错"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x212106_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x212106_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	AddMission( sceneId,selfId, x212106_g_MissionId, x212106_g_ScriptId, 1, 0, 0 )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x212106_g_MissionId)			--得到任务的序列号
 ...
 if objdataId == x212106_g_DemandKill[1].id then
	  ...
	  if num < x212106_g_DemandKill[1].num then
	    ...
	    SetMissionByIndex(sceneId,selfId,misIndex,1,num+1)
	  	BeginEvent(sceneId)
	  	strText = format("已杀死扬子鳄 %d/10", GetMissionParam(sceneId,selfId,misIndex,1) )
	  	AddText(sceneId,strText);
	  	EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	  end
