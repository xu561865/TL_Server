...
--任务需要杀死的怪
x212103_g_DemandKill ={{id=1620,num=8}}		--变量第1位

--以上是动态**************************************************************

--任务文本描述
x212103_g_MissionName="水鬼探子"
x212103_g_MissionInfo="杀死8个水鬼探子"  --任务描述
x212103_g_MissionTarget="杀死8个水鬼探子"		--任务目标
x212103_g_ContinueInfo="你已经杀了8个水鬼探子吗？"		--未完成任务的npc对话
x212103_g_MissionComplete="干的不错"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x212103_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x212103_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	AddMission( sceneId,selfId, x212103_g_MissionId, x212103_g_ScriptId, 1, 0, 0 )		--添加任务
 ...
 if objdataId == x212103_g_DemandKill[1].id then
	  ...
	  if num < x212103_g_DemandKill[1].num then
	    ...
	    SetMissionByIndex(sceneId,selfId,misIndex,1,num+1)
	  	BeginEvent(sceneId)
	  	strText = format("已杀死水鬼探子%d/8", GetMissionParam(sceneId,selfId,misIndex,1) )
	  	AddText(sceneId,strText);
	  	EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	  end
