...
--任务是否已经完成
x210603_g_IsMissionOkFail = 0		--变量的第0位

--以上是动态**************************************************************

--任务需要得到的物品
x210603_g_DemandItem={{id=40002092,num=5}}		--变量第1位

--任务文本描述
x210603_g_MissionName="蜂毒"
x210603_g_MissionInfo="得到5个大黄蜂的毒囊"  --任务描述
x210603_g_MissionTarget="得到5个大黄蜂的毒囊"		--任务目标
x210603_g_ContinueInfo="5个大黄蜂的毒囊你带来了么?"		--未完成任务的npc对话
x210603_g_MissionComplete="干得不错"					--完成任务npc说的话
...
--任务入口函数
--**********************************
function x210603_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x210603_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x210603_g_MissionId)			--得到任务的序列号
...
--杀死怪物或玩家
--**********************************
function x210603_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物objId
	if objdataId == 1515 then
		...
		for i=0,num-1 do
			...
			if IsHaveMission(sceneId,humanObjId,x210603_g_MissionId) > 0 then	--如果这个人拥有任务	
				if (GetItemCount(sceneId,humanObjId,x210603_g_ItemId) < x210603_g_ItemNeedNum) then
					AddMonsterDropItem(sceneId,objId,humanObjId,x210603_g_ItemId)    --给这个人任务道具(道具会出现在尸体包里)
				end
			end
		end
	end
...
function x210603_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x210603_g_ItemId then
		...
		if x210603_g_ItemNum < x210603_g_ItemNeedNum then
			BeginEvent(sceneId)
				strText = format("已得到大黄蜂的毒囊%d/5", x210603_g_ItemNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			--取得任务变量的值
		...
		elseif x210603_g_ItemNum == x210603_g_ItemNeedNum then
			...
			--显示得到物品数量
			BeginEvent(sceneId)
				strText = format("已得到大黄蜂的毒囊%d/5", x210603_g_ItemNeedNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
