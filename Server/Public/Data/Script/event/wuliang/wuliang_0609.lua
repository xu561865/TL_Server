...
--任务是否已经完成
x210609_g_IsMissionOkFail = 0		--变量的第0位


--任务需要杀死的怪
x210609_g_DemandKill ={{id=536,num=1}}		--变量第1位

--以上是动态**************************************************************

--任务文本描述
x210609_g_MissionName="刺杀特使"
x210609_g_MissionInfo="杀死神农帮特使"  --任务描述
x210609_g_MissionTarget="杀死神农帮特使"		--任务目标
x210609_g_ContinueInfo="你已经杀了神农帮特使吗？"		--未完成任务的npc对话
x210609_g_MissionComplete="干得不错"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x210609_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x210609_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	AddMission( sceneId,selfId, x210609_g_MissionId, x210609_g_ScriptId, 1, 0, 0 )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x210609_g_MissionId)			--得到任务的序列号
 ...
 if objdataId == x210609_g_DemandKill[1].id then
	  ...
	  if num < x210609_g_DemandKill[1].num then
	    ...
	    SetMissionByIndex(sceneId,selfId,misIndex,1,num+1)
	  	BeginEvent(sceneId)
	  	strText = format("已杀死神农帮特使%d/1", GetMissionParam(sceneId,selfId,misIndex,1) )
	  	AddText(sceneId,strText);
	  	EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	  end
