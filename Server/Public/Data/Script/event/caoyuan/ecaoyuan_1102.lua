...
--任务是否已经完成
x211102_g_IsMissionOkFail = 0		--变量的第0位

--任务需要杀死的怪
x211102_g_DemandKill = {{id=1825,num=5}}		--变量第1位

--以上是动态**************************************************************

--任务文本描述
x211102_g_MissionName="郡主的命令"
x211102_g_MissionInfo="去给我杀5个闪电马匪！少一个就别回来！"  --任务描述
x211102_g_MissionTarget="杀死5个闪电马匪"		--任务目标
x211102_g_ContinueInfo="真笨啊，快点去！"		--未完成任务的npc对话
x211102_g_MissionComplete="嗯？看来你有两下子嘛。"					--完成任务npc说话的话
...
x211102_g_DemandTrueKill = {{name="闪电马匪",num=5}}

--MisDescEnd
--**********************************
--任务入口函数
--**********************************
function x211102_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x211102_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	AddMission( sceneId,selfId, x211102_g_MissionId, x211102_g_ScriptId, 1, 0, 0 )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x211102_g_MissionId)			--得到任务的序列号
	...
	if GetName(sceneId,objId) == x211102_g_DemandTrueKill[1].name	  then
		...
		num = GetMissionParam(sceneId,selfId,misIndex,1)
			...
			SetMissionByIndex(sceneId,selfId,misIndex,1,num+1)
	  		BeginEvent(sceneId)
	  			strText = format("已杀死闪电马匪 %d/5", GetMissionParam(sceneId,selfId,misIndex,1) )
	  			AddText(sceneId,strText);
	  		EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
