...
--任务是否已经完成
x211101_g_IsMissionOkFail = 0		--变量的第0位
...
--任务文本描述
x211101_g_MissionName="黑蜂刺"
x211101_g_MissionInfo="自从由中原到了漠北，我就落下了腿痛的病根。 烈大哥教我用黑蜂来蜇膝盖，确实很有用。#r从前郡主和阿诚小的时候，他们总帮我去河边捉黑蜂来，现在他们都长大了，事情也多了，我真不忍心这么老折腾他们啊。#r"  --任务描述
x211101_g_MissionTarget="给阮老太太带回8根蒙古黑蜂的刺"		--任务目标
x211101_g_ContinueInfo="俺这个腿啊，是哇凉哇凉的啊"		--未完成任务的npc对话
x211101_g_MissionComplete="俺着老胳膊老腿终于有治了，你没让蜂蜇着吧？"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x211101_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x211101_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x211101_g_MissionId)			--得到任务的序列号
...
--杀死怪物或玩家
--**********************************
function x211101_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物objId
	if GetName(sceneId,objId) == "蒙古黑蜂"	  then
		num = GetMonsterOwnerCount(sceneId,objId)--取得这个怪物死后拥有分配权的人数
		for i=0,num-1 do
			humanObjId = GetMonsterOwnerID(sceneId,objId,i)--取得拥有分配权的人的objId
			if IsHaveMission(sceneId,humanObjId,x211101_g_MissionId) > 0 then	--如果这个人拥有任务	
				if (GetItemCount(sceneId,humanObjId,x211101_g_ItemId) < x211101_g_ItemNeedNum) then
					AddMonsterDropItem(sceneId,objId,humanObjId,x211101_g_ItemId)    --给这个人任务道具(道具会出现在尸体包里)
				end
			end
		end
	end
...
function x211101_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x211101_g_ItemId then
		...
		if x211101_g_ItemNum < x211101_g_ItemNeedNum then
			BeginEvent(sceneId)
				strText = format("已得到蒙古黑蜂的刺%d/8", x211101_g_ItemNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			--取得任务变量的值
		...
		elseif x211101_g_ItemNum == x211101_g_ItemNeedNum then
			...
			--显示得到物品数量
			BeginEvent(sceneId)
				strText = format("已得到蒙古黑蜂的刺%d/8", x211101_g_ItemNeedNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
