...
--任务文本描述
x210222_g_MissionName="新手：木人一巷"
x210222_g_MissionInfo="一个神秘的地下甬道，里边传出吱吱哑哑的声音"  --任务描述
x210222_g_MissionTarget="进入木人巷"	--任务目标
x210222_g_ContinueInfo="进入木人巷"	--未完成任务的npc对话
x210222_g_MissionComplete="出来就没事了"	--完成任务npc说话的话
x210222_g_SignPost = {x = 275, z = 50, tip = "黄眉僧"}

--任务奖励
x210222_g_MoneyBonus=1
...
x210222_g_Param_ok			=1	--1号：当前任务是否完成(0未完成；1完成)
...
x210222_g_Fuben_X=23				--进入副本的位置X
x210222_g_Fuben_Z=23				--进入副本的位置Z
x210222_g_Back_X=275				--源场景位置X
x210222_g_Back_Z=50					--源场景位置Z
x210222_g_TotalNeedKill=1000			--需要杀死怪物数量
...
--	
--	if	nearteammembercount<x210222_g_LimitMembers	then
--		BeginEvent(sceneId)
...
--			DispatchMissionTips(sceneId,selfId)
--			return 0
--		elseif IsHaveMission(sceneId,mems[i],x210222_g_MissionId)>0 then
--			--队友是否已经接过此任务或者另外一个任务
--			BeginEvent(sceneId)
...
function x210222_OnAccept( sceneId, selfId )
	...
	if( IsHaveMission(sceneId,selfId,x210222_g_MissionId) > 0)  then	--已经有任务的
		...
		--end
			--给每个队伍成员加入任务
			AddMission( sceneId, selfId, x210222_g_MissionId, x210222_g_ScriptId, 1, 0, 0 )
			...
			--将任务的第0号数据设置为已经完成的
			--SetMissionByIndex(sceneId,selfId,misIndex,x210222_g_Param_huan,huan)
			
			--将任务的第1号数据设置为0,表示未完成的任务
			SetMissionByIndex(sceneId,selfId,misIndex,x210222_g_Param_ok,0)
			...
			Msg2Player(  sceneId, selfId,"#Y接受任务：木人一巷",MSG2PLAYER_PARA )
			CallScriptFunction( SCENE_SCRIPT_ID, "AskTheWay", sceneId, selfId, SCENE_DALI, x210222_g_SignPost.x, x210222_g_SignPost.z, x210222_g_SignPost.tip )
...
--	mylevel = mylevel/nearmembercount
	...
	LuaFnSetSceneLoad_Monster(sceneId, "murenxiang_monster_7.ini")	--7级木人巷捉宠副本
...
--判断任务是否已经完成
	misIndex = GetMissionIndexByID(sceneId,selfId,x210222_g_MissionId)
	if	GetMissionParam( sceneId, selfId, misIndex, x210222_g_Param_ok)>=1 then 
		return	1
	else
		return	0
	end
...
function x210222_OnKillObject( sceneId, selfId, objdataId ,objId )
	...
	if fubentype~=x210222_g_CopySceneType then
		return
	end
	--副本关闭标志
	...
	if killednumber<x210222_g_TotalNeedKill then

		--BeginEvent(sceneId)
		--	strText = format("已杀 %d/%d", killednumber, x210222_g_TotalNeedKill )
		--	AddText(sceneId,strText);
		--EndEvent(sceneId)
		...
		for i=0,num-1 do
			...
			misIndex = GetMissionIndexByID(sceneId,humanObjId,x210222_g_MissionId) --取得任务数据索引值
			local killedcount = GetMissionParam( sceneId, humanObjId, misIndex, x210222_g_Param_killcount) --取得已经杀了的怪物数
			killedcount = killedcount +1 ;
			SetMissionByIndex(sceneId,humanObjId,misIndex,x210222_g_Param_killcount,killedcount) --设置任务数据
		end
	elseif killednumber>=x210222_g_TotalNeedKill then
		--设置任务完成标志
		...
		for i=0,num-1 do
			...
			SetMissionByIndex(sceneId,humanObjId,misIndex,x210222_g_Param_ok,1)--设置任务数据
			--完成副本所用时间
			SetMissionByIndex(sceneId,humanObjId,misIndex,x210222_g_Param_time,TickCount*x210222_g_TickTime)--设置任务数据

			BeginEvent(sceneId)
				strText = format("你结束了木人巷的探险，返回大理城！将在%d秒后传送到入口位置", x210222_g_CloseTick*x210222_g_TickTime )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,humanObjId)
		end
...
function x210222_OnHumanDie( sceneId, selfId, killerId )
	if x210222_g_DeadTrans==1 then --死亡后需要被强制踢出场景
	
		misIndex = GetMissionIndexByID(sceneId,selfId,x210222_g_MissionId)--取得任务数据索引值
		
		--将任务的第1号数据设置为1,表示完成的任务
		SetMissionByIndex(sceneId,selfId,misIndex,x210222_g_Param_ok,1)--设置任务数据
		
		--完成副本所用时间
		SetMissionByIndex(sceneId,selfId,misIndex,x210222_g_Param_time,TickCount*x210222_g_TickTime)--设置任务数据
	...
	if leaveFlag == 1 then --需要离开
		...
		elseif leaveTickCount<x210222_g_CloseTick then
			...
			for	i=0,membercount-1 do
				mems[i] = LuaFnGetCopyScene_HumanObjId(sceneId,i)
	  			BeginEvent(sceneId)
	  				strText = format("你将在%d秒后离开场景!", (x210222_g_CloseTick-leaveTickCount)*x210222_g_TickTime )
	  				AddText(sceneId,strText);
	  			EndEvent(sceneId)
	  			DispatchMissionTips(sceneId,mems[i])
			end
		end
	elseif TickCount == x210222_g_LimitTimeSuccess then
		--此处设置有时间限制的任务完成处理
		local membercount = LuaFnGetCopyScene_HumanCount(sceneId)
		local mems = {}
		for	i=0,membercount-1 do
			...
			misIndex = GetMissionIndexByID(sceneId,mems[i],x210222_g_MissionId)--取得任务数据索引值
			--将任务的第1号数据设置为1,表示完成的任务
			SetMissionByIndex(sceneId,mems[i],misIndex,x210222_g_Param_ok,1)--设置任务数据
			--完成副本所用时间
			SetMissionByIndex(sceneId,mems[i],misIndex,x210222_g_Param_time,TickCount*x210222_g_TickTime)--设置任务数据
		end
...
--		for	i=0,membercount-1 do
--			mems[i] = LuaFnGetCopyScene_HumanObjId(sceneId,i)
--			if IsHaveMission(sceneId,mems[i],x210222_g_MissionId) > 0 then
