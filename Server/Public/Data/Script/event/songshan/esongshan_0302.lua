...
--任务是否已经完成
x210302_g_IsMissionOkFail = 0		--变量的第0位

--任务需要杀死的怪
x210302_g_DemandKill = {{id=1600,num=10,name="遗弃铜人"}}		--变量第1位

--以上是动态**************************************************************

--任务文本描述
x210302_g_MissionName="遗弃铜人"
x210302_g_MissionTarget="杀死10个遗弃铜人"		--任务目标
x210302_g_ContinueInfo="真笨啊，快点去！"		--未完成任务的npc对话
x210302_g_MissionComplete="嗯？看来你有两下子嘛。"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x210302_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
	...
	elseif x210302_CheckAccept(sceneId, selfId) > 0 then --满足任务接收条件
		--发送任务接受时显示的信息
...
function x210302_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	print(format("接受了%s任务", x210302_g_MissionName));
	AddMission( sceneId, selfId, x210302_g_MissionId, x210302_g_ScriptId, 1, 0, 0 );		--添加任务
	misIndex = GetMissionIndexByID(sceneId, selfId, x210302_g_MissionId);			--得到任务的序列号
...
function x210302_OnKillObject( sceneId, selfId, objdataId )
	...
	print("玩家:" .. PlayerName);
 	if objdataId == x210302_g_DemandKill[1].id then
	  ...
	  BeginEvent(sceneId);
	  local strText = format("已杀死%s%d/%d", x210302_g_DemandKill[1].name, num+1, x210302_g_DemandKill[1].num );
	  AddText(sceneId, strText);
	  EndEvent(sceneId);
		DispatchMissionTips(sceneId, selfId);
	end
