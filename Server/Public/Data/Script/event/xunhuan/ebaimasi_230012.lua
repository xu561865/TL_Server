...
--任务环数
x230012_g_MissionRound	= 10			--记录循环任务变量的第10个数
...
x230012_g_MissionInfo="  阿弥陀佛...#r  寺里需要请你去找王积薪，挑战珍珑棋局[269,88]。"  --任务描述
x230012_g_MissionTarget="帮智清大师打通棋局。#r"		--任务目标
x230012_g_ContinueInfo="阿弥陀佛...#r少侠可否打通了棋局？"		--未完成任务的npc对话
x230012_g_MissionComplete="善哉善哉，少侠为人豪爽，关爱百姓，不爱钱财，日后必有一番作为。"					--完成任务npc说话的话
...
function x230012_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x230012_g_MissionId)			--得到任务的序列号
	...
	SetMissionByIndex(sceneId,selfId,misIndex,1,x230012_g_ScriptId)						--根据序列号把任务变量的第1位置为任务脚本号
	--得到环数
	...
	if x230012_CheckSubmit( sceneId, selfId, selectRadioId ) then
		...
		if ret > 0 then
			...
			if mod(x230012_g_MissionRound,10) == 0 then
				x230012_g_Exp = Level * 10 * 10										--等级+环数函数，受经验调节常数的影响
			else
				x230012_g_Exp = Level * mod(x230012_g_MissionRound,10) * 10
			end
			if	floor((x230012_g_MissionRound - 1) / 10) >=1  then
				x230012_g_Exp = x230012_g_Exp +50												--11~20环任务，每环额外增加50点经验
			end
			--增加经验值
			...
			--显示对话框
			BeginEvent(sceneId)
				AddText(sceneId,"恭喜你完成了任务，给你"..x230012_g_Exp.."点经验和"..x230012_g_Exp.."钱")
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		end
	end
