...
--任务是否已经完成
x230002_g_IsMissionOkFail = 0		--变量的第0位

--任务环数
x230002_g_MissionRound	= 10			--记录循环任务变量的第10个数

--任务需要得到的物品
x230002_g_DemandItem={{id=20101002,num=1}}		--从背包中计算
--以上是动态**************************************************************

--任务变量第一位用来存储随机得到的脚本号

--任务文本描述
x230002_g_MissionName="白马寺修行"
x230002_g_MissionInfo="  阿弥陀佛...#r  请少侠帮贫僧找来一株蒲黄。"  --任务描述
x230002_g_MissionTarget="帮智清大师找来一株蒲黄。#r"		--任务目标
x230002_g_ContinueInfo="  阿弥陀佛...#r  少侠可否把蒲黄带来了？"		--未完成任务的npc对话
x230002_g_MissionComplete="善哉善哉，少侠为人豪爽，关爱百姓，不爱钱财，日后必有一番作为。"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x230002_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x230002_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x230002_g_MissionId)			--得到任务的序列号
	SetMissionByIndex(sceneId,selfId,misIndex,0,0)						--根据序列号把任务变量的第0位置0 (任务完成情况)
	SetMissionByIndex(sceneId,selfId,misIndex,1,x230002_g_ScriptId)						--根据序列号把任务变量的第1位置为任务脚本号
	--得到环数
	...
	if x230002_CheckSubmit( sceneId, selfId, selectRadioId ) then
		...
		if	ret > 0 then
			...
			if mod(x230002_g_MissionRound,10) == 0 then
				x230002_g_Exp = Level * 10 * 10										--等级+环数函数，受经验调节常数的影响
			else
				x230002_g_Exp = Level * mod(x230002_g_MissionRound,10) * 10
			end
			if	floor((x230002_g_MissionRound - 1) / 10) >=1  then
				x230002_g_Exp = x230002_g_Exp +50												--11~20环任务，每环额外增加50点经验
			end
			--增加经验值
			...
			--显示对话框
			BeginEvent(sceneId)
				AddText(sceneId,"恭喜你完成了任务，给你"..x230002_g_Exp.."点经验和"..x230002_g_Exp.."钱")
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		else
			BeginEvent(sceneId)
				AddText(sceneId,"你的任务不能完成，你的蒲黄怎么突然不见了？")
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		end
	end
...
--杀死怪物或玩家
--**********************************
function x230002_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物objId
end
...
function x230002_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x230002_g_DemandItem[1].id then
		x230002_g_ItemNum = GetItemCount(sceneId,selfId,x230002_g_DemandItem[1].id)		--检测当前玩家任务物品1拥有的数量
		if x230002_g_ItemNum < x230002_g_DemandItem[1].num then
			BeginEvent(sceneId)
				strText = format("已得到蒲黄%d/1", x230002_g_ItemNum )
