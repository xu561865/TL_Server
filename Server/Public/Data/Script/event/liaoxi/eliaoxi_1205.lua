...
--任务是否已经完成
x211205_g_IsMissionOkFail = 0		--变量的第0位

--任务需要得到的物品
x211205_g_DemandKill = {{id=1845,num=10}}	--变量第1位

--以上是动态**************************************************************

--任务文本描述
x211205_g_MissionName="红衣马匪"
x211205_g_MissionInfo="帮我杀死10个红衣马匪"  --任务描述
x211205_g_MissionTarget="杀死10个红衣马匪"		--任务目标
x211205_g_ContinueInfo="你已经杀死10个红衣马匪了？"		--未完成任务的npc对话
x211205_g_MissionComplete="太谢谢你了"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x211205_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x211205_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	AddMission( sceneId,selfId, x211205_g_MissionId, x211205_g_ScriptId, 1, 0, 0 )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x211205_g_MissionId)			--得到任务的序列号
 ...
 if objdataId == x211205_g_DemandKill[1].id then
	  ...
	  if num < x211205_g_DemandKill[1].num then
	    ...
	    SetMissionByIndex(sceneId,selfId,misIndex,1,num+1)
	  	BeginEvent(sceneId)
	  	strText = format("已杀死红衣马匪 %d/10", GetMissionParam(sceneId,selfId,misIndex,1) )
	  	AddText(sceneId,strText);
	  	EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	  end
