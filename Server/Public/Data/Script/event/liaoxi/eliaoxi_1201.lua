...
--任务是否已经完成
x211201_g_IsMissionOkFail = 0		--变量的第0位

--以上是动态**************************************************************

--任务需要得到的物品
x211201_g_DemandItem={{id=40002068,num=10}}		--变量第1位

--任务文本描述
x211201_g_MissionName="白狼王"
x211201_g_MissionInfo="想不到狼皮这么好卖，我发财了，能不能再帮我弄10张狼皮，听说白狼皮更值钱。对，就再帮我弄10张白狼王的皮"  --任务描述
x211201_g_MissionTarget="得到10张白狼皮"		--任务目标
x211201_g_ContinueInfo="弄到白狼皮了么？"		--未完成任务的npc对话
x211201_g_MissionComplete="太谢谢你了"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x211201_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x211201_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x211201_g_MissionId)			--得到任务的序列号
...
--杀死怪物或玩家
--**********************************
function x211201_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物objId
	if objdataId == 1830 then
		...
		for i=0,num-1 do
			...
			if IsHaveMission(sceneId,humanObjId,x211201_g_MissionId) > 0 then	--如果这个人拥有任务	
				if (GetItemCount(sceneId,humanObjId,x211201_g_ItemId) < x211201_g_ItemNeedNum) then
					AddMonsterDropItem(sceneId,objId,humanObjId,x211201_g_ItemId)    --给这个人任务道具(道具会出现在尸体包里)
				end
			end
		end
	end
...
function x211201_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x211201_g_ItemId then
		...
		if x211201_g_ItemNum < x211201_g_ItemNeedNum then
			BeginEvent(sceneId)
				strText = format("已得到白狼皮%d/10", x211201_g_ItemNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			--取得任务变量的值
		...
		elseif x211201_g_ItemNum == x211201_g_ItemNeedNum then
			...
			--显示得到物品数量
			BeginEvent(sceneId)
				strText = format("已得到白狼皮%d/10", x211201_g_ItemNeedNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
