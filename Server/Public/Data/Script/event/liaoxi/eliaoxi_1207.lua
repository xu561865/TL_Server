...
--任务是否已经完成
x211207_g_IsMissionOkFail = 0		--变量的第0位

--任务需要得到的物品
x211207_g_DemandKill={{id=517,num=1}}	--变量第1位

--以上是动态**************************************************************

--任务文本描述
x211207_g_MissionName="马匪王"
x211207_g_MissionInfo="帮我杀死马匪王"  --任务描述
x211207_g_MissionTarget="杀死马匪王"		--任务目标
x211207_g_ContinueInfo="你已经杀死马匪王了？"		--未完成任务的npc对话
x211207_g_MissionComplete="太谢谢你了"					--完成任务npc说的话
...
--任务入口函数
--**********************************
function x211207_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x211207_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	AddMission( sceneId,selfId, x211207_g_MissionId, x211207_g_ScriptId, 1, 0, 0 )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x211207_g_MissionId)			--得到任务的序列号
 ...
 if objdataId == x211207_g_DemandKill[1].id then
	  ...
	  if num < x211207_g_DemandKill[1].num then
	    ...
	    SetMissionByIndex(sceneId,selfId,misIndex,1,num+1)
	  	BeginEvent(sceneId)
	  	strText = format("已杀死马匪王 %d/1", GetMissionParam(sceneId,selfId,misIndex,1) )
	  	AddText(sceneId,strText);
	  	EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	  end
