...
--任务是否已经完成
x210405_g_IsMissionOkFail = 0		--变量的第0位

--任务需要杀死的怪
x210405_g_DemandKill ={{id=1595,num=10}}		--变量第1位


--任务文本描述
x210405_g_MissionName="杀死华南虎"
x210405_g_MissionInfo="杀死10只华南虎"
x210405_g_MissionTarget="杀死10只华南虎"
x210405_g_ContinueInfo="你已经杀了10只华南虎？没有就继续吧"
x210405_g_MissionComplete="恭喜你杀完了"

--任务奖励
x210405_g_MoneyBonus=10200
x210405_g_Exp = 3000
x210405_g_ItemBonus={{id=30002001,num=1}}
x210405_g_DemandTrueKill ={{name="华南虎",num=10}}

--MisDescEnd
--**********************************
--任务入口函数
--**********************************
function x210405_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x210405_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	AddMission( sceneId,selfId, x210405_g_MissionId, x210405_g_ScriptId, 1, 0, 0 )	--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x210405_g_MissionId)	--得到任务的序列号
 ...
 if GetName(sceneId,objId) == x210405_g_DemandTrueKill[1].name	  then
	  ...
	  if num < x210405_g_DemandTrueKill[1].num then
	    ...
	    SetMissionByIndex(sceneId,selfId,misIndex,1,num+1)
	  	BeginEvent(sceneId)
			strText = format("已杀死华南虎%d/10", GetMissionParam(sceneId,selfId,misIndex,1) )
			AddText(sceneId,strText);
	  	EndEvent(sceneId)
	  	DispatchMissionTips(sceneId,selfId)
	  end
