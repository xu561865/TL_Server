...
--任务是否已经完成
x210406_g_IsMissionOkFail = 0		--变量的第0位

--任务需要杀死的怪
x210406_g_DemandKill ={{id=1605,num=10}}		--变量第1位


--任务文本描述
x210406_g_MissionName="借刀杀人"
x210406_g_MissionInfo="杀死10名稻草人"
x210406_g_MissionTarget="杀死10名稻草人"
x210406_g_ContinueInfo="你已经杀了10稻草人？没有就继续吧"
x210406_g_MissionComplete="恭喜你杀完了"

--任务奖励
x210406_g_MoneyBonus=10200
x210406_g_Exp = 3000
x210406_g_ItemBonus={{id=30002001,num=1}}
x210406_g_DemandTrueKill ={{name="稻草人",num=10}}

--MisDescEnd
--**********************************
--任务入口函数
--**********************************
function x210406_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x210406_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	AddMission( sceneId,selfId, x210406_g_MissionId, x210406_g_ScriptId, 1, 0, 0 )	--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x210406_g_MissionId)	--得到任务的序列号
 ...
 if GetName(sceneId,objId) == x210406_g_DemandTrueKill[1].name	  then
	  ...
	  if num < x210406_g_DemandTrueKill[1].num then
	    ...
	    SetMissionByIndex(sceneId,selfId,misIndex,1,num+1)
	  	BeginEvent(sceneId)
			strText = format("已杀死稻草人%d/10", GetMissionParam(sceneId,selfId,misIndex,1) )
			AddText(sceneId,strText);
	  	EndEvent(sceneId)
	  	DispatchMissionTips(sceneId,selfId)
	  end
