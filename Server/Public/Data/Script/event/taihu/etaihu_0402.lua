...
--任务是否已经完成
x210402_g_IsMissionOkFail = 0		--变量的第0位

--******以上是动态*****

--任务需要得到的物品
x210402_g_DemandItem={{id=40002114,num=1}}		--要改成蜡丸呀， 变量第1位

--任务文本描述
x210402_g_MissionName="一丝希望"
x210402_g_MissionInfo="杀死神秘的信使，拿回他身上的蜡丸"  --任务描述
x210402_g_MissionTarget="杀死神秘的信使，拿回他身上的蜡丸"		--任务目标
x210402_g_ContinueInfo="还没有完成，需要继续努力哟"		--未完成任务的npc对话
x210402_g_MissionComplete="嗯？看来你有两下子嘛。"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x210402_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x210402_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x210402_g_MissionId)		--得到任务的序列号
...
function x210402_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x210402_g_ItemId then
		...
		if x210402_g_ItemNum < x210402_g_ItemNeedNum then
			BeginEvent(sceneId)
				strText = format("已得到蜡丸%d/1", x210402_g_ItemNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			--取得任务变量的值
		...
		elseif x210402_g_ItemNum == x210402_g_ItemNeedNum then
			...
			--显示得到物品数量
			BeginEvent(sceneId)
				strText = format("已得到蜡丸%d/1", x210402_g_ItemNeedNum )
				AddText(sceneId,strText)
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
