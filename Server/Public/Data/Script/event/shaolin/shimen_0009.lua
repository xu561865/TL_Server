...
--任务是否已经完成
x220009_g_IsMissionOkFail = 0		--变量的第0位

--任务需要得到的物品
x220009_g_DemandItem={{id=30101013,num=1}}		--从背包中计算

--任务环数
x220009_g_MissionRound	= 11			--记录循环任务变量的第10个数
--以上是动态**************************************************************

--任务变量第一位用来存储随机得到的脚本号

--任务文本描述
x220009_g_MissionName="少林寺门派任务"
x220009_g_MissionInfo="我的包子怎么不见了，帮我找来。"  --任务描述
x220009_g_MissionTarget="帮慧方找到一个包子。"		--任务目标
x220009_g_ContinueInfo="你把包子带来了吗？"		--未完成任务的npc对话
x220009_g_MissionComplete="不愧是少林弟子"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x220009_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x220009_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x220009_g_MissionId)			--得到任务的序列号
	SetMissionByIndex(sceneId,selfId,misIndex,0,0)						--根据序列号把任务变量的第0位置0 (任务完成情况)
	SetMissionByIndex(sceneId,selfId,misIndex,1,x220009_g_ScriptId)						--根据序列号把任务变量的第1位置为任务脚本号
	--得到环数
	...
	if x220009_CheckSubmit( sceneId, selfId, selectRadioId ) then
		...
		if	ret > 0 then
			...
			if mod(x220009_g_MissionRound,10) == 0 then
				x220009_g_Exp = Level * 10 * 10										--等级+环数函数，受经验调节常数的影响
			else
				x220009_g_Exp = Level * mod(x220009_g_MissionRound,10) * 10
			end
			if	floor((x220009_g_MissionRound - 1) / 10) >=1  then
				x220009_g_Exp = x220009_g_Exp +50												--11~20环任务，每环额外增加50点经验
			end
			--增加经验值
			...
			--显示对话框
			BeginEvent(sceneId)
				AddText(sceneId,"恭喜你完成了任务，给你"..x220009_g_Exp.."点经验和"..x220009_g_Exp.."钱")
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		else
			BeginEvent(sceneId)
				AddText(sceneId,"你的任务不能完成，你的包子在哪里？")
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		end
	end
...
--杀死怪物或玩家
--**********************************
function x220009_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物objId
end
...
function x220009_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x220009_g_DemandItem[1].id then
		x220009_g_ItemNum = GetItemCount(sceneId,selfId,x220009_g_DemandItem[1].id)		--检测当前玩家任务物品1拥有的数量
		if x220009_g_ItemNum < x220009_g_DemandItem[1].num then
			BeginEvent(sceneId)
				strText = format("已得到包子%d/1", x220009_g_ItemNum )
