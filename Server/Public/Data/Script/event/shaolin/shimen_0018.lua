...
--任务是否已经完成
x220018_g_IsMissionOkFail = 0		--变量的第0位

--任务环数
x220018_g_MissionRound	= 11			--记录循环任务变量的第11个数
--以上是动态**************************************************************

--任务变量第一位用来存储随机得到的脚本号

--任务文本描述
x220018_g_MissionName="少林寺门派任务"
x220018_g_MissionInfo="开一个小佛舍利，把它送给卫天望"  --任务描述
x220018_g_MissionTarget="把小佛舍利送给卫天望"		--任务目标
x220018_g_ContinueInfo="你把舍利送到了吗？"		--未完成任务的npc对话
x220018_g_MissionComplete="不愧是少林弟子"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x220018_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x220018_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x220018_g_MissionId)			--得到任务的序列号
	SetMissionByIndex(sceneId,selfId,misIndex,0,0)						--根据序列号把任务变量的第0位置0 (任务完成情况)
	SetMissionByIndex(sceneId,selfId,misIndex,1,x220018_g_ScriptId)						--根据序列号把任务变量的第1位置为任务脚本号
	SetMissionByIndex(sceneId,selfId,misIndex,3,3)						--根据序列号把任务变量的第3位置为任务目标NPC编号，3为卫天望
	--得到环数
...
function x220018_OnSubmit( sceneId, selfId, targetId,selectRadioId )
	...
	if x220018_CheckSubmit( sceneId, selfId, selectRadioId ) then
		...
		if mod(x220018_g_MissionRound,10) == 0 then
			x220018_g_Exp = Level * 10 * 10										--等级+环数函数，受经验调节常数的影响
		else
			x220018_g_Exp = Level * mod(x220018_g_MissionRound,10) * 10
		end
		if	floor((x220018_g_MissionRound - 1) / 10) >=1  then
			x220018_g_Exp = x220018_g_Exp +50												--11~20环任务，每环额外增加50点经验
		end
		--增加经验值
		...
		--显示对话框
		BeginEvent(sceneId)
			AddText(sceneId,"恭喜你完成了任务，给你"..x220018_g_Exp.."点经验和"..x220018_g_Exp.."钱")
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	end
end
