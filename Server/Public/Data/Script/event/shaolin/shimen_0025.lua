...
--任务是否已经完成
x220025_g_IsMissionOkFail = 0		--变量的第0位

--任务需要得到的物品
x220025_g_DemandItem={{id=40002125,num=5}}		--从背包中计算

--任务环数
x220025_g_MissionRound	= 11			--记录循环任务变量的第10个数
--以上是动态**************************************************************

--任务变量第一位用来存储随机得到的脚本号

--任务文本描述
x220025_g_MissionName="少林寺门派任务"
x220025_g_MissionInfo="去寺里转转，拾点柴火回来，5个就行"  --任务描述
x220025_g_MissionTarget="帮慧方拾回5个柴火。"		--任务目标
x220025_g_ContinueInfo="你把柴火带来了吗？"		--未完成任务的npc对话
x220025_g_MissionComplete="不愧是少林弟子"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x220025_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x220025_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x220025_g_MissionId)			--得到任务的序列号
	SetMissionByIndex(sceneId,selfId,misIndex,0,0)						--根据序列号把任务变量的第0位置0 (任务完成情况)
	SetMissionByIndex(sceneId,selfId,misIndex,1,x220025_g_ScriptId)						--根据序列号把任务变量的第1位置为任务脚本号
	SetMissionByIndex(sceneId,selfId,misIndex,5,1)						--根据序列号把任务变量的第5位置为任务目标生长点编号，1为柴火
	--得到环数
...
function x220025_OnSubmit( sceneId, selfId, targetId,selectRadioId )
	...
	if x220025_CheckSubmit( sceneId, selfId, selectRadioId ) then
		...
		if mod(x220025_g_MissionRound,10) == 0 then
			x220025_g_Exp = Level * 10 * 10										--等级+环数函数，受经验调节常数的影响
		else
			x220025_g_Exp = Level * mod(x220025_g_MissionRound,10) * 10
		end
		if	floor((x220025_g_MissionRound - 1) / 10) >=1  then
			x220025_g_Exp = x220025_g_Exp +50												--11~20环任务，每环额外增加50点经验
		end
		--增加经验值
		...
		--显示对话框
		BeginEvent(sceneId)
			AddText(sceneId,"恭喜你完成了任务，给你"..x220025_g_Exp.."点经验和"..x220025_g_Exp.."钱")
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	end
end
...
--杀死怪物或玩家
--**********************************
function x220025_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物objId
end
...
function x220025_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x220025_g_DemandItem[1].id then
		x220025_g_ItemNum = GetItemCount(sceneId,selfId,x220025_g_DemandItem[1].id)		--检测当前玩家任务物品1拥有的数量
		if x220025_g_ItemNum < x220025_g_DemandItem[1].num then
			BeginEvent(sceneId)
				strText = format("已得到散落的柴火%d/5", x220025_g_ItemNum )
