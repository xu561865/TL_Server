...
--任务是否已经完成
x220013_g_IsMissionOkFail = 0		--变量的第0位

--任务需要得到的物品
x220013_g_DemandItem={{id=40002115,num=1}}		--从背包中计算

--任务环数
x220013_g_MissionRound	= 11			--记录循环任务变量的第10个数
--以上是动态**************************************************************

--任务变量第一位用来存储随机得到的脚本号

--任务文本描述
x220013_g_MissionName="少林寺门派任务"
x220013_g_MissionInfo="帮我把这封信送给破贪"  --任务描述
x220013_g_MissionTarget="帮慧方送信给破贪"		--任务目标
x220013_g_ContinueInfo="你把信送到了吗？"		--未完成任务的npc对话
x220013_g_MissionComplete="不愧是少林弟子"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x220013_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x220013_OnAccept( sceneId, selfId )
	...
	ret = EndAddItem(sceneId,selfId)
		--添加任务奖励
	if ret > 0 then
		misIndex = GetMissionIndexByID(sceneId,selfId,x220013_g_MissionId)			--得到任务的序列号
		SetMissionByIndex(sceneId,selfId,misIndex,0,0)						--根据序列号把任务变量的第0位置0 (任务完成情况)
		SetMissionByIndex(sceneId,selfId,misIndex,1,x220013_g_ScriptId)						--根据序列号把任务变量的第1位置为任务脚本号
		SetMissionByIndex(sceneId,selfId,misIndex,2,1)						--根据序列号把任务变量的第2位置为任务目标NPC编号，1为破贪
...
function x220013_OnSubmit( sceneId, selfId, targetId,selectRadioId )
	...
	if x220013_CheckSubmit( sceneId, selfId, selectRadioId ) then
		...
		if mod(x220013_g_MissionRound,10) == 0 then
			x220013_g_Exp = Level * 10 * 10										--等级+环数函数，受经验调节常数的影响
		else
			x220013_g_Exp = Level * mod(x220013_g_MissionRound,10) * 10
		end
		if	floor((x220013_g_MissionRound - 1) / 10) >=1  then
			x220013_g_Exp = x220013_g_Exp +50												--11~20环任务，每环额外增加50点经验
		end
		--增加经验值
		...
		--显示对话框
		BeginEvent(sceneId)
			AddText(sceneId,"恭喜你完成了任务，给你"..x220013_g_Exp.."点经验和"..x220013_g_Exp.."钱")
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	end
end
