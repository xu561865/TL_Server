...
--学习界面要说的话
x713527_g_MessageStudy = "如果你达到10级并且肯花费1两银子就可以学会药理技能。你决定学习么？"


--**********************************
--任务入口函数
--**********************************
function x713527_OnDefaultEvent( sceneId, selfId, targetId, ButtomNum,g_Npc_ScriptId )
	--玩家技能的等级
	...
	if ButtomNum == 0 then
		...
		--确定学习按钮
				AddNumText(sceneId,x713527_g_ScriptId,"我确定要学习", -1, 2)
		--取消学习按钮
				AddNumText(sceneId,x713527_g_ScriptId,"我只是来看看", -1, 3)
		EndEvent(sceneId)
...
--列举事件
--**********************************
function x713527_OnEnumerate( sceneId, selfId, targetId )
		--如果不到等级则不显示选项
		if GetLevel(sceneId,selfId) >= 10 then
			AddNumText(sceneId,x713527_g_ScriptId,"学习药理技能", -1, 0)
		end
		return
end
