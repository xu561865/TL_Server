...
--脚本号
x713603_g_ScriptId = 713514

--对话内容
x713603_g_dialog = {"俗语说:民以食为天。少侠在闯荡江湖时离不开美食的陪伴，但是美食的价格可不是很便宜，要是少侠自己学会了种植，就可以自己种出需要的材料，再也不会挨饿啦。",
...
x713603_g_button = {"种出的粮食是不是可以马上食用了?",
			"我怎样才能种植呢?",
			"然后呢?",
			"怎样收获呢?",
			}
...
--任务入口函数
--**********************************
function x713603_OnDefaultEvent( sceneId, selfId, targetId, MessageNum )	--MessageNum是对话编号，用于调用不同对话
		BeginEvent(sceneId)
			AddText(sceneId, x713603_g_dialog[MessageNum])
			if MessageNum ~= 5 then
				AddNumText(sceneId,MessageNum, x713603_g_button[MessageNum])
			end
		EndEvent(sceneId)
...
function x713603_OnEnumerate( sceneId, selfId, targetId )
		AddNumText(sceneId,x713603_g_ScriptId,"我想了解种植")
end
