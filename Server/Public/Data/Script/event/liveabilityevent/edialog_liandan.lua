...
--对话内容
x713618_g_dialog = {"如果你拥有一颗平常心，就会觉得采药是一件惬意的事情，拿上小镰刀，漫步在山间，伴着鸟语花香，在发现一株稀有药草的一刹那那种幸福的感受有的人永远也无法拥有，因为那些人心中只有钱，再美好的事物在他们心中也只不过是更值钱而已。",
			"第二句话",
...
function x713618_OnDefaultEvent( sceneId, selfId, targetId, MessageNum )	--MessageNum是对话编号，用于调用不同对话
		BeginEvent(sceneId)
			...
			if MessageNum ~= 5 then
				AddNumText(sceneId,MessageNum, x713618_g_button[MessageNum])
			end
		EndEvent(sceneId)
...
function x713618_OnEnumerate( sceneId, selfId, targetId )
		AddNumText(sceneId,x713618_g_ScriptId,"我想了解种植")
end
