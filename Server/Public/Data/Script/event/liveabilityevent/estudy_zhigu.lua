...
--学习界面要说的话
x713516_g_MessageStudy = "如果你达到10级并且肯花费1两银子就可以学会制蛊技能。你决定学习么？"
...
function x713516_OnDefaultEvent( sceneId, selfId, targetId, ButtomNum,g_Npc_ScriptId )
	...
	if ButtomNum == 0 then
		...
		--确定学习按钮
				AddNumText(sceneId,x713516_g_ScriptId,"我确定要学习", -1, 2)
		--取消学习按钮
				AddNumText(sceneId,x713516_g_ScriptId,"我只是来看看", -1, 3)
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
...
function x713516_OnEnumerate( sceneId, selfId, targetId )
		...
		if GetLevel(sceneId,selfId) >= 10 then
			AddNumText(sceneId,x713516_g_ScriptId,"学习制蛊技能", -1, 0)
		end
		return
end
