...
--任务是否已经完成
x211904_g_IsMissionOkFail = 0		--变量的第0位

--以上是动态**************************************************************

--任务需要物品
x211904_g_DemandItem={{id=40002079,num=1}}	--变量第1位

--任务文本描述
x211904_g_MissionName="藤甲贼元帅"
x211904_g_MissionInfo="杀死藤甲贼元帅，拿到龙纹银锁片"  --任务描述
x211904_g_MissionTarget="杀死藤甲贼元帅，拿到龙纹银锁片"		--任务目标
x211904_g_ContinueInfo="拿到龙纹银锁片了么？"		--未完成任务的npc对话
x211904_g_MissionComplete="太谢谢你了"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x211904_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x211904_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x211904_g_MissionId)			--得到任务的序列号
...
--杀死怪物或玩家
--**********************************
function x211904_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物objId
	if objdataId == 527 then
		...
		for i=0,num-1 do
			...
			if IsHaveMission(sceneId,humanObjId,x211904_g_MissionId) > 0 then	--如果这个人拥有任务	
				if (GetItemCount(sceneId,humanObjId,x211904_g_ItemId) < x211904_g_ItemNeedNum) then
					AddMonsterDropItem(sceneId,objId,humanObjId,x211904_g_ItemId)    --给这个人任务道具(道具会出现在尸体包里)
				end
			end
		end
	end
...
function x211904_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x211904_g_ItemId then
		...
		if x211904_g_ItemNum < x211904_g_ItemNeedNum then
			BeginEvent(sceneId)
				strText = format("已得到龙纹银锁片%d/1", x211904_g_ItemNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			--取得任务变量的值
		...
		elseif x211904_g_ItemNum == x211904_g_ItemNeedNum then
			...
			--显示得到物品数量
			BeginEvent(sceneId)
				strText = format("已得到龙纹银锁片%d/1", x211904_g_ItemNeedNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
