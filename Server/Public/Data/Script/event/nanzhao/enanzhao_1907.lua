...
--任务是否已经完成
x211907_g_IsMissionOkFail = 0		--变量的第0位
...
--任务文本描述
x211907_g_MissionName="勇士之王"
x211907_g_MissionInfo="杀死勇士之王，取回凤纹银锁片"  --任务描述
x211907_g_MissionTarget="取回凤纹银锁片"		--任务目标
x211907_g_ContinueInfo="拿到凤纹银锁片了么？"		--未完成任务的npc对话
x211907_g_MissionComplete="太谢谢你了"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x211907_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x211907_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x211907_g_MissionId)			--得到任务的序列号
...
--杀死怪物或玩家
--**********************************
function x211907_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物objId
	if objdataId == 528 then
		...
		for i=0,num-1 do
			...
			if IsHaveMission(sceneId,humanObjId,x211907_g_MissionId) > 0 then	--如果这个人拥有任务	
				if (GetItemCount(sceneId,humanObjId,x211907_g_ItemId) < x211907_g_ItemNeedNum) then
					AddMonsterDropItem(sceneId,objId,humanObjId,x211907_g_ItemId)    --给这个人任务道具(道具会出现在尸体包里)
				end
			end
		end
	end
...
function x211907_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x211907_g_ItemId then
		...
		if x211907_g_ItemNum < x211907_g_ItemNeedNum then
			BeginEvent(sceneId)
				strText = format("已得到凤纹银锁片%d/1", x211907_g_ItemNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			--取得任务变量的值
		...
		elseif x211907_g_ItemNum == x211907_g_ItemNeedNum then
			...
			--显示得到物品数量
			BeginEvent(sceneId)
				strText = format("已得到凤纹银锁片%d/1", x211907_g_ItemNeedNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
