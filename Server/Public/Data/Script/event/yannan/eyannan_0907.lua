...
--任务是否已经完成
x210907_g_IsMissionOkFail = 0		--变量的第0位

--任务需要杀死的怪
x210907_g_DemandKill ={{id=1695,num=10}}		--变量第1位

--以上是动态**************************************************************

--任务文本描述
x210907_g_MissionName="三打秦家寨"
x210907_g_MissionInfo="去杀死10个秦家寨亲兵，清除雁门关的后顾之忧"
x210907_g_MissionTarget="杀死10个秦家寨亲兵"
x210907_g_ContinueInfo="你已经杀了10个秦家寨亲兵吗？"
x210907_g_MissionComplete="恭喜你杀完了"
...
--任务入口函数
--**********************************
function x210907_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x210907_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	AddMission( sceneId,selfId, x210907_g_MissionId, x210907_g_ScriptId, 1, 0, 0 )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x210907_g_MissionId)			--得到任务的序列号
 ...
 if objdataId == x210907_g_DemandKill[1].id then
	  ...
	  if num < x210907_g_DemandKill[1].num then
	    ...
	    SetMissionByIndex(sceneId,selfId,misIndex,1,num+1)
	  	BeginEvent(sceneId)
	  	strText = format("已杀死秦家寨亲兵%d/10", GetMissionParam(sceneId,selfId,misIndex,1) )
	  	AddText(sceneId,strText);
	  	EndEvent(sceneId)
	  	DispatchMissionTips(sceneId,selfId)
	  end
