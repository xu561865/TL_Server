...
--任务是否已经完成
x210904_g_IsMissionOkFail = 0		--变量的第0位

--以上是动态**************************************************************

--任务需要得到的物品
x210904_g_DemandItem={{id=40002057,num=1}}		--变量第1位

--任务文本描述
x210904_g_MissionName="完美的灰熊皮"
x210904_g_MissionInfo="去杀一只灰熊，剥一张完美的灰熊皮给我，它的皮可值钱呢，现在看上去就是一个芍药"
x210904_g_MissionTarget="得到完美的灰熊皮"
x210904_g_ContinueInfo="你已经拿到灰熊皮了吗？"
x210904_g_MissionComplete="好皮好皮"
...
--任务入口函数
--**********************************
function x210904_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x210904_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x210904_g_MissionId)			--得到任务的序列号
...
function x210904_OnKillObject( sceneId, selfId, objdataId ,objId)
 if objdataId == 1680 then
		...
		for i=0,num-1 do
			...
			if IsHaveMission(sceneId,humanObjId,x210904_g_MissionId) > 0 then	--如果这个人拥有任务	
				if (GetItemCount(sceneId,humanObjId,x210904_g_ItemId) < x210904_g_ItemNeedNum) then
					AddMonsterDropItem(sceneId,objId,humanObjId,x210904_g_ItemId)    --给这个人任务道具(道具会出现在尸体包里)
				end
			end
		end
	end
...
function x210904_OnItemChanged( sceneId, selfId, itemdataId )
		...
		if x210904_g_ItemNum < x210904_g_ItemNeedNum then
			BeginEvent(sceneId)
				strText = format("已得到灰熊皮%d/1", x210904_g_ItemNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			--取得任务变量的值
		...
		elseif x210904_g_ItemNum == x210904_g_ItemNeedNum then
			...
			--显示得到物品数量
			BeginEvent(sceneId)
				strText = format("已得到灰熊皮%d/1", x210904_g_ItemNeedNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
