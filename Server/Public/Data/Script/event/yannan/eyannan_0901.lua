...
--任务是否已经完成
x210901_g_IsMissionOkFail = 0		--变量的第0位

--任务需要杀死的怪
x210901_g_DemandKill ={{id=1675,num=10}}		--变量第1位

--以上是动态**************************************************************

--任务文本描述
x210901_g_MissionName="豹害"
x210901_g_MissionInfo="我已经发现，附近的金钱豹经常袭击百姓。杀死10只金钱豹，为百姓除掉一害！"
x210901_g_MissionTarget="杀死10只金钱豹"
x210901_g_ContinueInfo="你已经杀了10只金钱豹吗？"
x210901_g_MissionComplete="恭喜你杀完了"
...
--任务入口函数
--**********************************
function x210901_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x210901_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	AddMission( sceneId,selfId, x210901_g_MissionId, x210901_g_ScriptId, 1, 0, 0 )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x210901_g_MissionId)			--得到任务的序列号
...
function x210901_OnKillObject( sceneId, selfId, objdataId )
	...
	if objdataId == x210901_g_DemandKill[1].id then
		...
		--设置打怪数量+1
			...
			BeginEvent(sceneId)
				strText = format("已杀死金钱豹%d/10", GetMissionParam(sceneId,selfId,misIndex,1) )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
