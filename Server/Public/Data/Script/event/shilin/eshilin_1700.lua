...
--任务是否已经完成
x211700_g_IsMissionOkFail = 0		--变量的第0位

--******以上是动态*****

--任务需要得到的物品
x211700_g_DemandItem={{id=40002057,num=5}}		--变量第1位

--任务文本描述
x211700_g_MissionName="过冬的准备"
x211700_g_MissionInfo="杀死棕熊，得到5块棕熊的毛皮，把它们交给圆月村的村长郑玄"  --任务描述
x211700_g_MissionTarget="杀死棕熊，得到5块棕熊的毛皮，把它们交给圆月村的村长郑玄"		--任务目标
x211700_g_ContinueInfo="还没有完成，需要继续努力哟"		--未完成任务的npc对话
x211700_g_MissionComplete="嗯？看来你有两下子嘛。"					--完成任务npc说话的话
...
--任务入口函数
--**********************************
function x211700_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x211700_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x211700_g_MissionId)		--得到任务的序列号
...
function x211700_OnKillObject( sceneId, selfId, objdataId ,objId)
	if objdataId == 1860 then
		...
		for i=0,num-1 do
			...
			if IsHaveMission(sceneId,humanObjId,x211700_g_MissionId) > 0 then	--如果这个人拥有任务	
				if (GetItemCount(sceneId,humanObjId,x211700_g_ItemId) < x211700_g_ItemNeedNum) then
					AddMonsterDropItem(sceneId,objId,humanObjId,x211700_g_ItemId)    --给这个人任务道具(道具会出现在尸体包里)
				end
			end
		end
	end
...
function x211700_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x211700_g_ItemId then
		...
		if x211700_g_ItemNum < x211700_g_ItemNeedNum then
			BeginEvent(sceneId)
				strText = format("已得到熊皮%d/5", x211700_g_ItemNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			--取得任务变量的值
		...
		elseif x211700_g_ItemNum == x211700_g_ItemNeedNum then
			...
			--显示得到物品数量
			BeginEvent(sceneId)
				strText = format("已得到熊皮%d/5", x211700_g_ItemNeedNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
