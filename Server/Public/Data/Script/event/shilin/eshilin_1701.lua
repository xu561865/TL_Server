...
--任务是否已经完成
x211701_g_IsMissionOkFail = 0		--变量的第0位

--任务需要杀死的怪
x211701_g_DemandKill ={{id=1865,num=10}}		--变量第1位

--******以上是动态******

--任务文本描述
x211701_g_MissionName="石林中的狼蛛"
x211701_g_MissionInfo="我已经发现，附近的西北狼经常袭击百姓。杀死10只西北狼，为百姓除掉一害！"
x211701_g_MissionTarget="为圆月村的杨莲子杀死10只狼蛛"
x211701_g_ContinueInfo="你已经杀了10只狼蛛？没有就继续吧"
x211701_g_MissionComplete="恭喜你杀完了"
...
--任务入口函数
--**********************************
function x211701_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x211701_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	AddMission( sceneId,selfId, x211701_g_MissionId, x211701_g_ScriptId, 1, 0, 0 )	--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x211701_g_MissionId)	--得到任务的序列号
	...
	if objdataId == x211701_g_DemandKill[1].id then
		...
		--设置打怪数量+1
			SetMissionByIndex(sceneId,selfId,misIndex,1,num+1)
				BeginEvent(sceneId)
		strText = format("已杀死狼蛛%d/10", GetMissionParam(sceneId,selfId,misIndex,1) )
					AddText(sceneId,strText);
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,selfId)
		end
