...
--任务是否已经完成
x212006_g_IsMissionOkFail = 0		--变量的第0位
...
--任务文本描述
x212006_g_MissionName="护身符III"
x212006_g_MissionInfo="得到万劫修罗的护身符"  --任务描述
x212006_g_MissionTarget="得到万劫修罗的护身符"		--任务目标
x212006_g_ContinueInfo="万劫修罗的护身符你带来了么?"		--未完成任务的npc对话
x212006_g_MissionComplete="干得不错"					--完成任务npc说的话
...
function x212006_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x212006_g_MissionId)			--得到任务的序列号
...
--杀死怪物或玩家
--**********************************
function x212006_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物objId
	if objdataId == 2010 then
		...
		for i=0,num-1 do
			...
			if IsHaveMission(sceneId,humanObjId,x212006_g_MissionId) > 0 then	--如果这个人拥有任务	
				if (GetItemCount(sceneId,humanObjId,x212006_g_ItemId) < x212006_g_ItemNeedNum) then
					AddMonsterDropItem(sceneId,objId,humanObjId,x212006_g_ItemId)    --给这个人任务道具(道具会出现在尸体包里)
				end
			end
		end
	end
...
function x212006_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x212006_g_ItemId then
		...
		if x212006_g_ItemNum < x212006_g_ItemNeedNum then
			BeginEvent(sceneId)
				strText = format("已得到万劫修罗的护身符%d/1", x212006_g_ItemNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			--取得任务变量的值
		...
		elseif x212006_g_ItemNum == x212006_g_ItemNeedNum then
			...
			--显示得到物品数量
			BeginEvent(sceneId)
				strText = format("已得到万劫修罗的护身符%d/1", x212006_g_ItemNeedNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
