...
--任务是否已经完成
x212005_g_IsMissionOkFail = 0		--变量的第0位
...
--任务文本描述
x212005_g_MissionName="护身符II"
x212005_g_MissionInfo="得到永夜修罗的护身符"  --任务描述
x212005_g_MissionTarget="得到永夜修罗的护身符"		--任务目标
x212005_g_ContinueInfo="永夜修罗的护身符你带来了么?"		--未完成任务的npc对话
x212005_g_MissionComplete="干得不错"					--完成任务npc说的话
...
function x212005_OnAccept( sceneId, selfId )
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x212005_g_MissionId)			--得到任务的序列号
...
--杀死怪物或玩家
--**********************************
function x212005_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物objId
	if objdataId == 2005 then
		...
		for i=0,num-1 do
			...
			if IsHaveMission(sceneId,humanObjId,x212005_g_MissionId) > 0 then	--如果这个人拥有任务	
				if (GetItemCount(sceneId,humanObjId,x212005_g_ItemId) < x212005_g_ItemNeedNum) then
					AddMonsterDropItem(sceneId,objId,humanObjId,x212005_g_ItemId)    --给这个人任务道具(道具会出现在尸体包里)
				end
			end
		end
	end
...
function x212005_OnItemChanged( sceneId, selfId, itemdataId )
	if itemdataId == x212005_g_ItemId then
		...
		if x212005_g_ItemNum < x212005_g_ItemNeedNum then
			BeginEvent(sceneId)
				strText = format("已得到永夜修罗的护身符%d/1", x212005_g_ItemNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			--取得任务变量的值
		...
		elseif x212005_g_ItemNum == x212005_g_ItemNeedNum then
			...
			--显示得到物品数量
			BeginEvent(sceneId)
				strText = format("已得到永夜修罗的护身符%d/1", x212005_g_ItemNeedNum )
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
