...
function x714086_OnDefaultEvent( sceneId, selfId,targetId )
	...
	if AbilityLevel ~= 0	then
		BeginEvent(sceneId)
			...
			for i, eventId in x714086_g_eventList do	--遍历这个稻草人可以种植的植物列表
				for j,g_ZhiWuId in V_ZHONGZHI_ID do		--遍历scriptglobal中的所有植物列表
					if eventId == g_ZhiWuId then
						if AbilityLevel >= V_ZHONGZHI_NEEDLEVEL[j] then --如果玩家种植技能等级>=该植物要求技能等级]
							AddNumText(sceneId, x714086_g_eventList[i], V_ZHONGZHI_NAME[j])
							break
						end
					end
				end
...
--事件列表选中一项
--**********************************
function x714086_OnEventRequest( sceneId, selfId, targetId, eventId )
	zhiwuId = eventId
