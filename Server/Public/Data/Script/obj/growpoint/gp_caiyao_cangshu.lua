...
function 		x710514_OnCreate(sceneId,growPointType,x,y)
	--放入ItemBox同时放入一个物品
	targetId  = ItemBoxEnterScene(x,y,growPointType,sceneId,1,x710514_g_MainItemId)	--每个生长点最少能得到一个物品,这里直接放入itembox中一个
	--获得1~3的随机数,如果是1则不需要放入,如果多余1再用AddItemToBox增加物品
	ItemCount = random(1,3)
	if ItemCount ~= 1 then
		for i=1, (ItemCount - 1) do
			AddItemToBox(sceneId,targetId,1,x710514_g_MainItemId)
		end
	end
	--放入次要产品
	if random(1,10) == 1 then
		AddItemToBox(sceneId,targetId,1,x710514_g_SubItemId)
	end	
end
--生成函数结束**********************************************************************
...
function	x710514_OpenCheck(sceneId,selfId,AbilityId,AblityLevel)
	--检查生活技能等级
	if AbilityLevel<x710514_g_AbilityLevel then
		return OR_NO_LEVEL
	end
	--检查精力
	if GetHumanEnergy(sceneId,selfId)< floor(x710514_g_AbilityLevel * 1.5 +2) then
