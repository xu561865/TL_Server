...
function x002041_UpdateEventList( sceneId, selfId,targetId )
	...
	if Menpai == OR_WUMENPAI then
		AddText(sceneId,"  在下张获，武当山张真人门下。#r#r  武当一派的武功博大精深，我们师兄弟也同心同德，你原意拜在武当门下吗？#r#r  武当以内功见长，剑短气长，百丈外至敌于死地。")
	elseif Menpai == OR_WUDANG then
	...
	if	GetLevel( sceneId, selfId)<=10	then
		AddNumText(sceneId,x002041_g_scriptId,"去武当山看看",-1,0)
	end
	...
	if	IsHaveMission(sceneId,selfId,1060) > 0	 then
		if	GetMissionParam( sceneId, selfId, misIndex,0) < 1  and	 GetMissionParam( sceneId, selfId, misIndex,2) == 3   then
			AddNumText(sceneId,x002041_g_scriptId,"这是我师兄给你的信",2,1)
		end
	end
